...
-(void)createHeaderView
{
    ...
    if ([self.typeString isEqualToString:@"1"]) {
        ...
        //距离结束还有
        titleLable = [ZYControl createLabelWithFrame:CGRectMake(kScreenWidth/2-100*RATIOTYPE_WIDTH,34*RATIOTYPE_HEIGHT,250*RATIOTYPE_WIDTH,30*RATIOTYPE_HEIGHT) Font:16 Text:nil];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //        LogInViewController *lvc = [[LogInViewController alloc]init];
    //        [self.navigationController pushViewController:lvc animated:YES];
    //        lvc.typeString = self.typeString;
    //        for (PatProduceModel *model in _dataArray) {
    //            lvc.strID = model.uid;
    //        }
    //
