...
-(void)createDataBank
{
    ...
    NSDictionary *paramer = @{@"client_id":[user objectForKey:@"client_id"],@"orderid":_orderID};
    NSString *url = @"http://123.57.212.63/paimai/index.php?con=api&act=remittance";
    
    [manager POST:url parameters:paramer success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"银行转账／汇款数据返回成功 －－－ %@",responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        NSLog(@"银行转账／汇款数据返回失败 －－－ %@",operation.error);
    }];
