...
@interface AuctionViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    ...
    //记录点击cell的下标
    int index;
    DoneTableViewCell *doneCell;//已拍下的cell
...
- (void)viewDidLoad {
    ...
    auTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if (sign == 2)
    {
        ...
        doneCell = [tableView dequeueReusableCellWithIdentifier:auCell];
        ...
        doneCell.stateButton.tag = indexPath.row;
        ...
        if ([doneCell.sig isEqualToString:@"2"]) {
            [doneCell.stateButton setTitle:@"去支付" forState:UIControlStateNormal];
            [doneCell.stateButton setTitleColor:[AOColorFormat colorWithHexString:@"#ffffff"] forState:UIControlStateNormal];
            doneCell.stateButton.backgroundColor = [AOColorFormat colorWithHexString:@"#fe0000"];
            
            
        }else if ([doneCell.sig isEqualToString:@"3"])
        {
            [doneCell.stateButton setTitle:@"确认收货地址" forState:UIControlStateNormal];
            [doneCell.stateButton setTitleColor:[AOColorFormat colorWithHexString:@"#ffffff"] forState:UIControlStateNormal];
            doneCell.stateButton.backgroundColor = [AOColorFormat colorWithHexString:@"#c70065"];
        }else if ([doneCell.sig isEqualToString:@"5"])
        {
            [doneCell.stateButton setTitle:@"确认收货" forState:UIControlStateNormal];
            [doneCell.stateButton setTitleColor:[AOColorFormat colorWithHexString:@"#ffffff"] forState:UIControlStateNormal];
            doneCell.stateButton.backgroundColor = [AOColorFormat colorWithHexString:@"#c70065"];
        
        }else
        {
            [doneCell.stateButton setTitle:@"查看详情" forState:UIControlStateNormal];
            [doneCell.stateButton setTitleColor:[AOColorFormat colorWithHexString:@"#ffffff"] forState:UIControlStateNormal];
            doneCell.stateButton.backgroundColor = [AOColorFormat colorWithHexString:@"#c70065"];
...
{
    NSArray *arr = _dataArr3;
    
    NSDictionary *dic = arr[button.tag];

    NSString *str = [NSString stringWithFormat:@"%@",[dic objectForKey:@"order_num"]];
        ...
        if ([str isEqualToString:@"2"]) {
            ...
            payVC.orderID = [NSString stringWithFormat:@"%@",[dic objectForKey:@"orderid"]];
        ...
        }else if ([str isEqualToString:@"3"])
        {
            ...
            addressVC.orderID = [NSString stringWithFormat:@"%@",[dic objectForKey:@"orderid"]];
        ...
        }else
        {
            ...
            NSArray *array = [[NSArray alloc]init];
            array = _dataArr3;
            detailVC.detailDic =array[index];
            [self.navigationController pushViewController:detailVC animated:YES];
        }
