...
-(void)createDrawData
{
    ...
    [manager POST:url parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSDictionary *dic = [drawDic objectForKey:@"account"];
...
//        CashDetailModel *model = [[CashDetailModel alloc]init];
       ...
       self.money.text = [NSString stringWithFormat:@"%@",[drawDic objectForKey:@"deposit_thaw"]];
        self.bankCard.text = [NSString stringWithFormat:@"%@",[dic objectForKey:@"brand_cardnumber"]];
        self.bankName.text = [NSString stringWithFormat:@"%@",[dic objectForKey:@"brand_cardname"]];
        self.name.text = [NSString stringWithFormat:@"%@",[dic objectForKey:@"brand_holder"]];
        [self.bankBg sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@" ,[dic objectForKey:@"img"]]]];
...
-(void)createView
{
    ...
    UILabel *label4 = [[UILabel alloc]initWithFrame:CGRectMake(2, 30 * RATIOTYPE_HEIGHT, 60, 20)];
    label4.text = @"退款账户";
